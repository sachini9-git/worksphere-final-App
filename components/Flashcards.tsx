...
export const Flashcards: React.FC<FlashcardsProps> = ({ documents, addDocument }) => {
  ...
  const [autoGeneratedIds, setAutoGeneratedIds] = useState<Set<string>>(new Set());
  const [lastDocCount, setLastDocCount] = useState<number>(documents.length);

  const generateCards = async (docId?: string, cards?: number) => {
    const targetDocId = docId || selectedDoc;
    if (!targetDocId) return;

    setIsLoading(true);
    const doc = documents.find((d) => d.id === targetDocId);
    if (doc) {
      const cards_to_generate = cards || cardCount;
      const cards_result = await generateFlashcards(doc, cards_to_generate);
      if (cards_result.length > 0) {
        setFlashcards(cards_result);
        setCurrentIndex(0);
        setIsFlipped(false);
  ...
  // Auto-generate flashcards for newly uploaded documents with real content
  useEffect(() => {
    const studyDocs = documents.filter(d => d.type !== 'flashcard_set' && d.type !== 'quiz_set');
    
    // Detect if a new document was added
    if (studyDocs.length > lastDocCount) {
      const newDoc = studyDocs[studyDocs.length - 1];
      
      // Check if document has real extracted content (not placeholder)
      const isRealContent = newDoc.content && newDoc.content.trim().length > 100 && 
        !newDoc.content.toLowerCase().includes('click "edit" above and paste') &&
        !newDoc.content.toLowerCase().includes('file attached');
      
      // Auto-generate if not already done for this doc
      if (isRealContent && !autoGeneratedIds.has(newDoc.id)) {
        setAutoGeneratedIds(prev => new Set(prev).add(newDoc.id));
        generateCards(newDoc.id, 5);
      }
    }
    
    setLastDocCount(studyDocs.length);
  }, [documents]);
          ...
          <div className="space-y-4">
            ...
            {selectedDoc && documents.find(d => d.id === selectedDoc)?.content && (
              <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 max-h-48 overflow-y-auto">
                <p className="text-xs font-bold text-blue-700 mb-2 uppercase tracking-wider">✓ Content Preview</p>
                <p className="text-sm text-blue-900 leading-relaxed line-clamp-6">
                  {documents.find(d => d.id === selectedDoc)?.content?.substring(0, 400) || 'No content'}...
                </p>
                <p className="text-xs text-blue-600 mt-2 font-medium">Your file was successfully read!</p>
              </div>
            )}
