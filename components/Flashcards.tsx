...
export const Flashcards: React.FC<FlashcardsProps> = ({ documents, addDocument }) => {
  ...
  useEffect(() => {
    ...
    if (studyDocs.length > lastDocCount) {
      ...
      // Only auto-generate if document has reasonable content (not just empty)
      // Backend validation will handle detailed checks
      const hasContent = newDoc.content && newDoc.content.trim().length > 50;
      
      // Auto-generate if not already done for this doc
      if (hasContent && !autoGeneratedIds.has(newDoc.id)) {
