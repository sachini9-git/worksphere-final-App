...
export const Flashcards: React.FC<FlashcardsProps> = ({ documents, addDocument }) => {
  ...
  const generateCards = async (docId?: string, cards?: number, isAutoGenerate: boolean = false) => {
    ...
    if (doc) {
      ...
      const cards_result = await generateFlashcards(doc, cards_to_generate, !isAutoGenerate);
  ...
  useEffect(() => {
    ...
    if (studyDocs.length > lastDocCount) {
      ...
      // Check if document has real extracted content (strict validation)
      ...
      ].some(phrase => newDoc.content.toLowerCase().includes(phrase.toLowerCase()));

      // Only auto-generate if:
      // 1. Content is substantial (150+ chars of real content, not just header)
      // 2. No placeholder phrases detected
      const lines = newDoc.content.trim().split('\n');
      const substantialContent = lines.filter(line => line.trim().length > 0).slice(1).join('\n');
      const isRealContent = !hasPlaceholder && substantialContent.trim().length > 150;
      ...
      if (isRealContent && !autoGeneratedIds.has(newDoc.id)) {
        ...
        generateCards(newDoc.id, 5, true);
      }
    }
